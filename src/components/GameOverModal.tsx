      ...
      <div className="w-full flex flex-wrap gap-1.5 sm:gap-2 items-center justify-center pt-1.5 sm:pt-2">
        {actionButtons.map((btn, idx) => {
          ...
          return (
            ...
            >
              {isPrimary && <ArrowRight className="w-3 h-3 sm:w-3.5 sm:h-3.5 order-2" />}
              {isRetry && <RotateCcw className="w-3 h-3 sm:w-3.5 sm:h-3.5" />}
              {btn.id === 'menu' && <Home className="w-3 h-3 sm:w-3.5 sm:h-3.5" />}
              <span>{btn.label}</span>
            </button>
          );
        })}
